...
class EncoderText(nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        x = self.embed(x)
        # print('embedding_type:',type(x),lengths)
        caplens=[]
        for i in lengths:
            caplens.append(i)
        caplens.sort(reverse=True)
        packed = pack_padded_sequence(x, caplens, batch_first=True)
...
class EncoderMultiText(nn.Module):

    def __init__(self, vocab_size, word_dim, embed_size, num_layers,
                 use_bi_gru=False, no_txtnorm=False):
        super(EncoderMultiText, self).__init__()
        self.embed_size = embed_size
        self.no_txtnorm = no_txtnorm
    ...
    def forward(self, input, lengths):
        ...
        cap_emb_all = torch.LongTensor(input.shape[0],input.shape[1],input.shape[2],self.embed_size).fill_(0)
        cap_len_all = torch.LongTensor(input.shape[0],input.shape[1])
        for ind in range(input.shape[1]):
            x_i = input[:,ind,:]
            caplen_i = lengths[:,ind]
            x = torch.squeeze(x_i).long()
            # Embed word ids to vectors
            x = self.embed(x)
            # print('embedding_type:',type(x),lengths)
            caplens = []
            for i in caplen_i:
                caplens.append(i)
            caplens.sort(reverse=True)
            # print('embedding_x.shape:',x.shape)
            packed = pack_padded_sequence(x, caplens, batch_first=True)
            ...
            cap_emb_all[:,ind,:,:]=cap_emb
            cap_len_all[:,ind]=cap_len

        return cap_emb_all, cap_len_all
...
def func_attention(query, context, opt, smooth, eps=1e-8):
    ...
    # print(type(context),type(queryT),context.size(),queryT.size())
