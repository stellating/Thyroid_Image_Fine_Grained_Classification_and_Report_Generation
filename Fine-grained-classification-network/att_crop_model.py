...
class Encoder_crop(nn.Module):
    ...
    def __init__(self, encoded_image_size=9):
        super(Encoder_crop, self).__init__()
        self.enc_image_size = encoded_image_size
...
class Attention1(nn.Module):
    ...
    def forward(self, h, att_feats, p_att_f):
        ...
        p_att_feats = torch.cat([p_att_f,p_att_f],1)
...
class DecoderWithAttention(nn.Module):
    ...
    def forward(self, encoder_out, encoder_crop_out, encoder_feature, encoded_captions, caption_lengths):
        """
        Forward propagation.
        ...
        encoder_crop_out = encoder_crop_out.view(batch_size, -1, encoder_dim)
        ...
        encoder_crop_out = encoder_crop_out[sort_ind]
        ...
        num_pixels = encoder_crop_out.size(1)

        # Initialize LSTM state
        # (batch_size, decoder_dim)
        h_lang, c_lang = self.init_hidden_state(entire_encoder_out)
        ...
        for t in range(max(decode_lengths)):
            ...
            attention_weighted_encoding, alpha = self.attention(encoder_crop_out[:batch_size_t],
                                                                h_lang[:batch_size_t])
            ...
            att = self.attention1(h_att, entire_encoder_out[:batch_size_t], encoder_feature[:batch_size_t])
