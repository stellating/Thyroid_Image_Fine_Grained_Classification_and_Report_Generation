...
data_name = 'thyroid_5_cap_per_img_5_min_word_freq'  # base name shared by data files
checkpoint = './checkpoint/BEST-att-alpha-checkpoint_thyroid_5_cap_per_img_5_min_word_freq.pth.tar'  # model checkpoint
word_map_file = './caption_data/WORDMAP_thyroid_5_cap_per_img_5_min_word_freq.json'  # word map, ensure it's the same the data was encoded with and the model was trained with
...
def evaluate(beam_size):
    ...
    # For each image
    for i, (img_idx, image, caps, caplens, allcaps) in enumerate(
        ...
        p_conv_feats=encoder_feats(image).view(1, -1, encoder_dim)
        p_conv_feats=p_conv_feats.expand(k, num_pixels, encoder_dim)
        ...
        while True:
            ...
            awe, _ = decoder.attention(encoder_out, h_lang)  # (s, encoder_dim), (s, num_pixels)
